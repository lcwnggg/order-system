-- ══════════════════════════════════════════════════════════════
-- Coste de compra + proveedor, PRIVADO (solo el almacén / la dueña)
-- ══════════════════════════════════════════════════════════════
--
-- Por qué una tabla aparte y no dos columnas en `products`:
--   `products` tiene una policy de SELECT abierta a cualquier usuario
--   autenticado (las tiendas necesitan ver nombre/precio/stock), y el cliente
...
--   Con una tabla propia + RLS de solo-almacén, el empleado recibe 0 filas.
--
...
-- reservados al rol `warehouse`. Las tiendas no ven ni una fila.
...
-- ── Comprobación ──
--   SELECT count(*) FROM public.product_costs;                 -- como almacén: OK
