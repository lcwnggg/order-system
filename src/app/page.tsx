    ...
    <div className="min-h-screen bg-sage-100">
      ...
      <header className="border-b border-sage-200 bg-white px-6 py-4">
        <div className="mx-auto flex max-w-5xl items-center justify-between">
          <span className="text-sm font-semibold text-sage-900">我的小店</span>
          <div className="flex items-center gap-2 sm:gap-3">
            {isWarehouse && (
              <div className="hidden items-center gap-2 sm:flex">
                <Link
                  href="/admin/products"
                  className="rounded-lg bg-sage-700 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-sage-800"
                >
                  商品管理
                </Link>
                <Link
      ...
      {isWarehouse && (
        <main className="mx-auto max-w-5xl space-y-6 px-6 py-8">
          {/* 欢迎 */}
          <div>
            <h1 className="text-2xl font-semibold text-sage-900">欢迎回来！</h1>
            <p className="mt-1 text-sm text-sage-500">{dateStr}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
            <Link
              href="/admin/orders"
              className={`group rounded-2xl border bg-sage-25 p-5 shadow-[0_4px_16px_rgba(91,107,87,0.06)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_28px_rgba(91,107,87,0.13)] ${
                pendingCount > 0 ? "border-amber-200" : "border-sage-200"
              }`}
            >
              <div className="flex items-center justify-between">
                <span className={`flex h-9 w-9 items-center justify-center rounded-xl ${pendingCount > 0 ? "bg-amber-100 text-amber-600" : "bg-sage-100 text-sage-500"}`}>
                ...
                <span className="text-sage-300 transition group-hover:translate-x-0.5 group-hover:text-sage-500">→</span>
              </div>
              <p className={`mt-3 text-3xl font-bold ${pendingCount > 0 ? "text-amber-600" : "text-sage-400"}`}>{pendingCount}</p>
              <p className="mt-0.5 text-xs font-medium text-sage-500">待处理订单</p>
            </Link>

            <Link
              href="/admin/products"
              className="group rounded-2xl border border-sage-200 bg-sage-25 p-5 shadow-[0_4px_16px_rgba(91,107,87,0.06)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_28px_rgba(91,107,87,0.13)]"
            >
              <div className="flex items-center justify-between">
              ...
              <p className="mt-3 text-3xl font-bold text-green-600">{productCount}</p>
              <p className="mt-0.5 text-xs font-medium text-sage-500">商品总数</p>
            </Link>

            <Link
              href="/admin/stock-alert"
              className={`group rounded-2xl border bg-sage-25 p-5 shadow-[0_4px_16px_rgba(91,107,87,0.06)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_28px_rgba(91,107,87,0.13)] ${
                lowStockCount > 0 ? "border-red-200" : "border-sage-200"
              }`}
            >
              <div className="flex items-center justify-between">
                <span className={`flex h-9 w-9 items-center justify-center rounded-xl ${lowStockCount > 0 ? "bg-red-100 text-red-600" : "bg-sage-100 text-sage-500"}`}>
              ...
              <p className={`mt-3 text-3xl font-bold ${lowStockCount > 0 ? "text-red-600" : "text-sage-400"}`}>{lowStockCount}</p>
              <p className="mt-0.5 text-xs font-medium text-sage-500">库存告急 <span className="text-sage-400">≤ 5</span></p>
            </Link>
            ...
            >
              ...
              <p className="mt-3 text-3xl font-bold text-blue-600">{storeCount}</p>
              <p className="mt-0.5 text-xs font-medium text-sage-500">门店数</p>
            </Link>
            ...
            >
              ...
              <p className="mt-3 text-3xl font-bold text-purple-600">{categoryCount}</p>
              <p className="mt-0.5 text-xs font-medium text-sage-500">商品大类</p>
            </Link>
          </div>
        </main>
      )}
      ...
      {isStore && (
        <main className="mx-auto flex max-w-lg flex-col items-center px-6 py-24 text-center">
          <div className="mb-8 flex h-20 w-20 items-center justify-center rounded-3xl bg-sage-700 shadow-[0_10px_28px_rgba(74,90,70,0.35)]">
          ...
          <p className="mt-2 text-sm text-sage-500">准备好下单了吗？浏览商品，加入购物车，一键提交。</p>
          <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row">
            <Link
              href="/shop"
              className="rounded-xl bg-sage-700 px-8 py-4 text-base font-semibold text-white shadow-[0_8px_20px_rgba(74,90,70,0.3)] transition-colors hover:bg-sage-800"
            >
              进入商城下单 →
            </Link>
            <Link
              href="/shop/orders"
              className="rounded-xl border border-sage-200 bg-sage-25 px-6 py-4 text-base font-medium text-sage-700 transition-colors hover:bg-sage-100"
            >
              我的订单
            </Link>
          </div>
      ...
      {!user && (
        <main className="mx-auto flex max-w-md flex-col items-center px-6 py-24 text-center">
          ...
          <h1 className="text-2xl font-semibold text-sage-900">欢迎使用我的小店</h1>
          <p className="mt-2 text-sm text-sage-500">供应商与分店的订货管理平台。请登录以继续。</p>
          <Link
            href="/login"
            className="mt-8 rounded-xl bg-sage-700 px-8 py-4 text-base font-semibold text-white shadow-[0_8px_20px_rgba(74,90,70,0.3)] transition-colors hover:bg-sage-800"
          >
            立即登录
          </Link>
        </main>
