  ...
  if (stores.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-zinc-300 bg-white py-16 text-center">
        <p className="text-sm text-zinc-400">暂无 store 角色用户</p>
      </div>
    );
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-zinc-100 bg-zinc-50 text-left">
            <th className="px-5 py-3 font-medium text-zinc-500">邮箱</th>
            <th className="px-5 py-3 font-medium text-zinc-500">店名</th>
          ...
          {stores.map((store) => {
            ...
            return (
              <tr key={store.id} className="hover:bg-zinc-50">
                <td className="px-5 py-3 text-zinc-600">{store.email}</td>
                <td className="px-5 py-3">
                  <input
                    ...
                    placeholder="输入店名…"
                    className="w-full max-w-xs rounded-lg border border-zinc-200 px-3 py-1.5 text-sm text-zinc-900 placeholder-zinc-400 outline-none focus:border-zinc-400 focus:ring-2 focus:ring-zinc-100"
                  />
                </td>
                <td className="px-5 py-3">
