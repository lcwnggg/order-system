...
type NavItem = { href: string; labelKey: TranslationKey; d: string };
...
const STORE_NAV: NavItem[] = [
  { href: "/", labelKey: "nav.home", d: "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" },
  { href: "/shop", labelKey: "nav.shop", d: "M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" },
  { href: "/transfers", labelKey: "nav.transfers", d: "M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" },
  { href: "/shop/orders", labelKey: "nav.myOrders", d: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" },
];
      ...
      <div className="glass mx-auto flex min-h-[calc(100vh-1.5rem)] max-w-[1240px] flex-col overflow-hidden rounded-[26px] sm:min-h-[calc(100vh-2.5rem)] md:grid md:grid-cols-[228px_1fr]">
        ...
        <aside className="flex flex-col gap-1 border-b border-white/40 bg-gradient-to-b from-white/40 to-white/10 p-4 md:border-b-0 md:border-r">
          <Link href="/" className="flex items-center gap-2.5 px-2 pb-4 pt-1">
            <span className="flex h-7 w-7 items-center justify-center rounded-[9px] bg-gradient-to-br from-accent-500 to-[#8f7fd8] text-sm text-white">{t("app.logoLetter")}</span>
            <span className="text-[17px] font-semibold text-paper-900">{t("app.name")}</span>
          </Link>
            ...
            {nav.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {t(item.labelKey)}
                  {item.href === "/transfers" && <TransferNavBadge />}
                </Link>
              );
            })}
          </nav>
          {/* 账号 + 退出：不能用 mt-auto 顶到底部——侧栏在 grid 里会被拉到和主内容一样高，
              仪表盘那种长页面会把它推到屏幕外，等于没有退出入口 */}
          <div className="mt-4 flex items-center gap-2.5 rounded-2xl bg-white/40 p-2.5">
            <span className="h-9 w-9 shrink-0 rounded-full bg-gradient-to-br from-accent-200 to-[#d8b8cf]" />
            <div className="min-w-0 flex-1">
              <p className="truncate text-[13px] font-medium text-paper-900">{displayName ?? (variant === "store" ? t("role.store") : t("role.warehouse"))}</p>
              <p className="truncate text-[11px] text-paper-400">{email}</p>
            </div>
            <form action={signOut}>
              <button type="submit" aria-label={t("auth.signOut")} className="rounded-lg p-1.5 text-paper-400 transition-colors hover:bg-white/60 hover:text-paper-700">
          ...
          <div className="mt-2 flex justify-end px-1 text-paper-500">
            <LanguageSwitcher />
          </div>
        </aside>
